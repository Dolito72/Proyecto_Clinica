...
public class OdontologoJpaController implements Serializable {
    ...
    public void create(Odontologo odonto) {
    ...
    public void edit(Odontologo odonto) throws NonexistentEntityException, Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            ...
            if (msg == null || msg.length() == 0) {
                ...
                if (findOdontologo(id) == null) {
                    throw new NonexistentEntityException("The odonto with id " + id + " no longer exists.");
                }
            }
            throw ex;
        } finally {
    ...
    public void destroy(int id) throws NonexistentEntityException {
        ...
        try {
            ...
            Odontologo odonto;
            try {
                odonto = em.getReference(Odontologo.class, id);
                odonto.getId();
            } catch (EntityNotFoundException enfe) {
                throw new NonexistentEntityException("The odonto with id " + id + " no longer exists.", enfe);
            }
            em.remove(odonto);
            em.getTransaction().commit();
        } finally {
